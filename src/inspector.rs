...
fn inspect_route(extract_path: &Path)
    ...
    let mut file_map: HashMap<String, PathBuf> = HashMap::new();// 返回
    let mut layer_hash_set: HashSet<String> = HashSet::new();// 返回
    ...
    let expr_config = Regex::new(r#"^[a-z0-9]{64}.json$"#)?;
    let expr_layer_and_config = Regex::new(r#"^[a-z0-9]{64}"#)?;
    ...
    if config_num == 0 {
        Error::BadDockerFileError { msg: format!("no config.json") };
    } else if manifest_path.components().next().is_none() {
        Error::BadDockerFileError { msg: format!("no manifest.json") };
    } else if repositories_path.components().next().is_none() {
        Error::BadDockerFileError { msg: format!("no repositories") };
    }
...
fn inspect_config(file_map: &HashMap<String, PathBuf>)
    ...
    let config_path = Path::new(file_map
        .get("config_path")
        .ok_or_else(|| Error::KeyError { key: format!("config_path") })?);
    let config_filestem = config_path
        .file_stem()
        .ok_or_else(|| Error::FilePathError { path: config_path.to_path_buf() })?
        .to_os_string()
        .into_string()
        .map_err(|_| Error::ConvertError())?;
    ...
    let diff_ids = match diff_ids {
        ...
        _ => {
            Err(Error::BadDockerFileError { msg: format!("config file parse failed") })
        }
    }?;
...
fn inspect_layer(extract_path: &Path,
    ...
    for layer in layer_hash_set {
        ...
        let layer_paths = read_dir(&layer_dir_path)?;
        ...
        if !has_json {
            Error::BadDockerFileError { msg: format!("no json inside layer '{:?}'", layer) };
        } else if !has_layer {
            Error::BadDockerFileError { msg: format!("no layer.tar inside layer '{:?}'", layer) };
        }
    }
    // 对称差
...
fn inspect_manifest(extract_path: &Path,
    ...
    let manifest_path = file_map
        .get("manifest_path")
        .ok_or_else(|| Error::KeyError { key: format!("manifest_path") })?;
    let config = load_config(manifest_path)?;
    let config_array = match &config {
        JsonValue::Array(config_item) => { Ok(config_item) }
        _ => { Err(Error::BadDockerFileError { msg: format!("manifest file parse failed") }) }
    }?;
    ...
    for layer in layers {
        ...
        let layer_path = layer_parent_path
            .parent()
            .ok_or_else(|| Error::FilePathError { path: layer_parent_path.to_path_buf() })?;
        if !layer_hash_set
            .contains(layer_path.to_str().ok_or_else(|| Error::ConvertError())?) {
            Error::BadDockerFileError { msg: format!("layer inside manifest doesn't exist") };
        }
...
pub fn inspect<P>(extract_path: P)
    ...
    let (file_map, layer_hash_set) = inspect_route(extract_path)?;
